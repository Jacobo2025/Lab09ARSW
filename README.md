...
1. Diríjase a el [Portal de Azure](https://portal.azure.com/) y a continuación cree una maquina virtual con las características básicas descritas en la imágen 1 y que corresponden a las siguientes:
    ...
    * SSH public key = Su llave ssh publica
...
### Evidencia
Antes de conectarnos, tuvimos que descargar la clave .pem y darle permisos de ejecución.
...
5. Para ejecutar la aplicación puede usar el comando `node FibonacciApp.js`, sin embargo una vez pierda la conexión ssh la aplicación dejará de funcionar. Para evitar ese comportamiento usaremos *forever*. Ejecute los siguientes comandos dentro de la VM.
...
6. Antes de verificar si el endpoint funciona, en Azure vaya a la sección de *Networking* y cree una *Inbound port rule* tal como se muestra en la imágen. Para verificar que la aplicación funciona, use un browser y use el endpoint `http://xxx.xxx.xxx.xxx:3000/fibonacci/6`. La respuesta debe ser `The answer is 8`.
...
7. La función que calcula el enésimo número de la secuencia de Fibonacci está muy mal construida y consume bastante CPU para obtener la respuesta. Usando la consola del Browser documente los tiempos de respuesta para dicho endpoint usando los siguientes valores:
    * 1000000
    * 1010000
    * 1020000
    * 1030000
...
9. Ahora usaremos Postman para simular una carga concurrente a nuestro sistema. Siga estos pasos.
    ...
    * Diríjase hasta la ruta `FibonacciApp/postman` en una máquina diferente a la VM.
...
### Respuesta
Sí, porque al redefinir el tamaño aumentamos la capacidad de procesamiento de la máquina virtual. Esto se puede evidenciar en las capturas anteriores, donde los tiempos de respuesta fueron menores y el consumo de CPU también disminuyó.
...
- **NSG:** Firewall que controla el tráfico entrante/saliente mediante reglas de puerto, protocolo e IP.
- **OS Disk:** Disco duro virtual donde está instalado el sistema operativo de la VM.
...
`node FibonacciApp.js` corre como proceso hijo de la sesión SSH. Al cerrar la conexión, el proceso padre muere y arrastra todos sus hijos con él. Por eso se usa `forever`, que corre la app como proceso independiente del sistema, desacoplado de la sesión SSH.

El Inbound port rule es necesario porque el NSG bloquea todo el tráfico entrante por defecto. Sin una regla que abra el puerto 3000, Azure descarta los paquetes antes de que lleguen a la VM, haciendo el servicio inaccesible desde internet.
...
Cada petición tarda en promedio 20 segundos, lo cual se explica por dos razones principales. Primero, la VM B1ls tiene recursos extremadamente limitados (1 vCPU y 0.5 GiB de RAM), lo que restringe severamente la capacidad de procesamiento. Segundo, y más importante, el algoritmo de Fibonacci implementado usa recursión simple con complejidad exponencial O(2^n). Esto significa que para calcular el enésimo número, la función se llama a sí misma millones de veces recalculando los mismos valores repetidamente sin guardar resultados intermedios (sin memoización), lo que hace el proceso extremadamente ineficiente para valores grandes como 1.000.000.
...
No es la mejor solución a largo plazo. Es escalabilidad vertical, que tiene un límite físico y un costo creciente. Cuando se cambia el tamaño de la VM, Azure reinicia la máquina, lo que provoca downtime y hace que la FibonacciApp deje de responder durante el proceso de redimensionamiento. Si no se usa `forever` o un proceso similar, hay que volver a iniciar la app manualmente después del reinicio.

---

**9. ¿Qué pasa con la infraestructura cuando cambia el tamaño de la VM? ¿Qué efectos negativos implica?**

- La VM se reinicia completamente, generando downtime.
- La aplicación deja de responder durante el cambio (puede tardar varios minutos).
...
1. El Balanceador de Carga es un recurso fundamental para habilitar la escalabilidad horizontal de nuestro sistema.
...
Ahora vamos a crear 3 VMs (VM1, VM2 y VM3) con direcciones IP públicas estándar en 3 diferentes zonas de disponibilidad. Después las agregaremos al balanceador de carga.

1. En la configuración básica de la VM guíese por la siguiente imágen. Es importante que se fije en la "Availability Zone", donde la VM1 será 1, la VM2 será 2 y la VM3 será 3.
...
Realice este proceso para las 3 VMs, por ahora lo haremos a mano una por una. Sin embargo es importante saber que existen herramientas para automatizar este proceso, entre ellas: Azure Resource Manager, OsDisk Images, Terraform con Vagrant y Packer, Puppet, Ansible, entre otras.

### Evidencia

> **IMPORTANTE:** Solo se usaron 2 máquinas virtuales debido a que la suscripción de estudiante no permitió crear una tercera máquina por restricciones de cuota y política de regiones disponibles.
...
1. El endpoint de acceso a nuestro sistema es la IP pública del balanceador de carga. Primero verifiquemos que los servicios básicos están funcionando:
...
2. Realice las pruebas de carga con `newman` y compare con la Parte 1.

![alt text](images_solution/image10.png)

---

3. Agregue una 4ta máquina virtual y realice las pruebas de newman con 4 instancias paralelas.
...
> **Nota:** Este punto no pudo realizarse debido a las limitaciones de cuota de la suscripción de estudiante, que no permitió agregar una cuarta máquina virtual. Teóricamente, con 4 VMs la carga se distribuiría entre más nodos, reduciendo el consumo de CPU por máquina y aumentando significativamente la tasa de éxito de las peticiones, ya que cada VM atendería aproximadamente 1/4 del total de solicitudes concurrentes.
...
La persistencia es importante cuando la aplicación guarda estado en memoria local (sesiones de usuario, caché). Sin embargo, reduce la escalabilidad porque amarra usuarios a instancias específicas, impidiendo una distribución equitativa de carga y limitando el beneficio del escalamiento horizontal. Para APIs sin estado como FibonacciApp, la opción óptima es **None**.
...
**Informe de Newman - Punto 2 (comparativo escalamiento vertical vs horizontal)**
...
| Downtime por mantenimiento | Sí (al redimensionar) | No (se puede sacar una VM sin afectar el servicio) |
